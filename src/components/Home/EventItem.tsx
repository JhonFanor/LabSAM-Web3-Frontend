...
export const EventItem: React.FC<Props> = ({ event }) => (
    <Link to={`/event/${event.id}`} className="event-item__list-item">
        ...
        <p className="event-item__list-item-user">
            Subido por: <img src={event.user.avatar || "/src/assets/img/avatar.png"} alt="icono" className="avatar_img"/> {event.user.regular_user?.name || event.user.university_user?.name || event.user.business_user?.name || "Anónimo"}
        </p>
    </Link>
);
