...
import { getAllJobsBoardNotApproved } from "../../api";
import { Pagination, GetAllError } from "../../components";
import "./GetAllJobBoard.css";

export const GetAllJobsBoardNotApproved: React.FC = () => {
    ...
    useEffect(() => {
        const getJobBoard = async () => {
            try {
                const data = await getAllJobsBoardNotApproved(page, limit);
        ...
        <section className="get-all-job-board">
            ...
            <div className="get-all-job-board__list">
                {jobBoardList.map((jobBoard) => (
                    <Link to={`/admin/job-board/${jobBoard.id}`} key={jobBoard.id} className="get-all-job-board__list-item">
