...
export const DocumentInputSelector: React.FC<DocumentInputSelectorProps> = ({ value, onChange, onFileSelected, urlLabel, fileLabel, documentUploaderKey, resetKey }) => {
	const [documentOption, setDocumentOption] = useState<"url" | "file">(value.startsWith("http") ? "url" : "file");

	useEffect(() => {
		setDocumentOption("url");
	}, [resetKey]);
